...
class PillTong {
  ...
  constructor(options) {
    ...
    // set filters
    this.filters = _.map(this.conf.filters, (filterName) => importCwd(PillTong.modulePathResolver(filterName)));
  ...
  /**
   * get module path safety
   * 
   * @param {String} src path
   * 
   * @return {String} resolved path
   */
  static modulePathResolver(src) {
    const cwd = process.cwd();

    // local module
    if (PillTong.isFileAvailable(path.resolve(cwd, src.replace(/(\.js)?$/, '.js')))) {
      return src;
    }

    // package module
    if (PillTong.isFileAvailable(path.resolve(cwd, 'node_modules', src))) {
      return path.resolve(process.cwd(), 'node_modules', src);
    }

    throw new Error(`${src} module not found`);
  }
}
